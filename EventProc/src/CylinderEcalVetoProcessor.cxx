/**
 * @file CylinderEcalVetoProcessor.cxx
...
#include "EventProc/CylinderEcalVetoProcessor.h"

namespace ldmx {

    CylinderEcalVetoProcessor::CylinderEcalVetoProcessor(const std::string &name, Process &process):
        Producer(name, process) { 
    }

    CylinderEcalVetoProcessor::~CylinderEcalVetoProcessor() {
        delete hexReadout_; 
    }

    void CylinderEcalVetoProcessor::configure(const ParameterSet& ps) {
        //verbose_ = ps.getBool("verbose",false);
    }

    std::vector<SimParticle*> CylinderEcalVetoProcessor::getRecoilElectrons(const TClonesArray* simParticles) { 
    ...
    std::vector<SimTrackerHit*> CylinderEcalVetoProcessor::getRecoilElectronHcalSPHits(
    ...
    return hits;  
    //std::vector<SimTrackerHit*> CylinderEcalVetoProcessor::getRecoilElectrons(Event& event){
    ...
    void CylinderEcalVetoProcessor::produce(Event& event) {
        ...
        for(const auto& electron : recoils) {
            ...
            for (int iHit = 0; iHit < nEcalHits; iHit++) {
                ...
                //std::cout << " [ CylinderEcalVetoProcessor ] : rawid " << rawID << " layer " << layer << " cellID " << cellID << " moduleID " << moduleID << " combineid " << combinedID << std::endl;
                std::pair<double,double> hitPosition = hexReadout_->getCellCenterAbsolute(combinedID);
                //std::cout << " [ CylinderEcalVetoProcessor ] : hit position: " << hitPosition.first << " " << hitPosition.second << std::endl;
       ...
       // event.addToCollection("CylinderEcalVetoProcessor", //result_);
    }
}

DECLARE_PRODUCER_NS(ldmx, CylinderEcalVetoProcessor);
