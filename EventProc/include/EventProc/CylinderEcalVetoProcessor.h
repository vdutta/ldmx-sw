...
namespace ldmx {
    ...
    class CylinderEcalVetoProcessor: public Producer {

        public:

            /** Constructor */
            CylinderEcalVetoProcessor(const std::string& name, Process& process);

            /** Destructor */
            ~CylinderEcalVetoProcessor(); 
        ...
        private:
            ...
            void Debug(std::string output){ 
                if( verbose_ )
                    std::cout << " [ CylinderEcalVetoProcessor ] : " << output << std::endl;
            };

            template <class T> void Debug(std::string output_label,std::vector<T> output_value){ 
                if( verbose_ ){
                    std::cout << " [ CylinderEcalVetoProcessor ] : " << output_label ;
